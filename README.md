...
As you can see Apple's primary example leads you to a dead end. You could augment it with an embedded webserver, but that's more work. The alternative using a XPC Launch Daemon has a one to many relationship vs. an XPCService plugin is only one to one.
